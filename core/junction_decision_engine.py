...
class JunctionDecisionEngine:
    ...
    def should_prioritize_lane_feasible_now(
        ...
        aged_pending = int(pending_age) >= 1
    ...
    def should_cancel_pending_early(
        ...
        lane_progress = False
        if prev_lane_idx is not None and prev_shift is not None:
            lane_progress = abs(curr_shift) < abs(int(prev_shift))
        road_progress = (
            prev_dist is not None
            and (float(prev_dist) - float(context.dist_to_end)) >= self.pending_min_progress_m
        )
