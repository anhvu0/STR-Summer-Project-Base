...
class RLTrainingPipeline:
    ...
    def _select_fallback_action(self, context, blocked_action):
        fallback_actions = self.decision_engine.lane_feasible_fallback_actions(context, blocked_action=blocked_action)
        if fallback_actions:
            return fallback_actions[0]
        connected = self.decision_engine.safe_connected_fallback_actions(context, blocked_action=blocked_action)
        if connected:
            return connected[0]
        return None
                    ...
                    for vehicle_id in controlled_live_ids:
                        ...
                        if vehicle_id in pending_decisions and current_edge != pending_decisions[vehicle_id].decision_edge:
                            ...
                            if pending_phase == "observe_lane_change":
                                ...
                                fallback_action = self._select_fallback_action(obs_context, blocked_action=pending.intended_action)
                        ...
                        if not safe_ok:
                            ...
                            action = self._select_fallback_action(context, blocked_action=action)
                            if action is None:
                                prev_edge_by_vehicle[vehicle_id] = current_edge
                                continue
                            decision_metrics["fallback_overrides"] += 1
                        ...
                        if action not in context.lane_feasible_now_actions:
                            ...
                            if step < cooldown_until:
                                ...
                                fallback_action = self._select_fallback_action(context, blocked_action=action)
                                if fallback_action is None:
                                    prev_edge_by_vehicle[vehicle_id] = current_edge
                                    continue
