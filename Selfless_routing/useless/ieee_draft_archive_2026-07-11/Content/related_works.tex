...
minimizes total cost~\cite{wardrop1952}. Roughgarden and Tardos quantified how far apart
these can be, proving bounds on the price of anarchy for selfish
routing~\cite{roughgarden2002selfish}, and the Braess paradox shows the equilibrium can
be counter-intuitively bad~\cite{braess2005paradox}. The gap is not merely theoretical:
Zhang et al.~\cite{zhang2018poa} estimated the price of anarchy from real traffic data on
...
Dai et al.~\cite{dai2019str} introduced the selfless driving model and argued that
...
state that later actions face~\cite{mnih2015human}. Proximal Policy Optimization (PPO)
provides a stable on-policy actor--critic update via a clipped surrogate
objective~\cite{schulman2017ppo}, and Generalized Advantage Estimation (GAE) reduces the
variance of the advantage signal it optimizes~\cite{schulman2016gae}. In the multi-agent
...
local~\cite{lowe2017maddpg}. MAPPO applies PPO under CTDE with a centralized value
function and has proven surprisingly strong on cooperative
benchmarks~\cite{yu2022mappo}; we use it here because our reward is cooperative and our
...
global objective~\cite{wolpert2001collectives}, and counterfactual multi-agent policy
gradients (COMA) estimate that marginal effect with a centralized
critic~\cite{foerster2018coma}. Our team-reward term is a leave-one-out difference reward
in travel-time units: each agent pays for its own slowness relative to the live fleet
average, so the exogenous, demand-driven congestion level---which no single vehicle
controls---cancels out and the learnable signal is sharpened.

\subsection{Simulation substrate}
We use Eclipse SUMO as the microscopic traffic environment~\cite{lopez2018sumo,krajzewicz2012recent}.
...
commit-window constraints. Dijkstra's algorithm~\cite{dijkstra1959note} remains the
