...
which gridlocks this map) and runs each controller through StrSumo, exactly like
RLTrainingPipeline._run_frozen_inference_eval.run_eval_controller.

Reference points at this demand (diag_braess_due_so.py 240 1.5):
    T_static ~ 409s (all-braess) ,  T_DUE ~ 406s (selfish eq) ,  T_SO ~ 303s (optimum)
A policy that "shows selfless routing" must push avg TT below T_DUE toward T_SO.
...
SEEDS = [7000 + i for i in range(N_SEEDS)]   # held out from training/frozen-eval seeds
SPAWN = 1.5
...
pipe = RLTrainingPipeline(
    ...
    team_reward_alpha=1.0, team_reward_mode="difference",
)
SUMO = checkBinary("sumo")
NET = os.path.join(pipe.sumocfg_dir, pipe.net_file)


def run_ctrl(controller, vehicles):
...
arms = {"MAPPO-greedy": [], "MAPPO-stoch": [], "Dijkstra-static": []}
print(f"model={MODEL}  seeds={SEEDS}  spawn={SPAWN}  (240 veh, pattern 4)\n")
print(f"{'seed':>6} {'MAPPO-greedy':>13} {'MAPPO-stoch':>12} {'Dijkstra':>10} {'MAPPO nonzero%':>14}")
for seed in SEEDS:
    ...
    s_mg = run_ctrl(mg, copy.deepcopy(vehicles))
    ...
    s_ms = run_ctrl(ms, copy.deepcopy(vehicles))

    dj = DijkstraPolicy(pipe.connection_info)
    s_dj = run_ctrl(dj, copy.deepcopy(vehicles))
    ...
    print(f"{seed:>6} {s_mg['avg_travel_time']:>13.1f} {s_ms['avg_travel_time']:>12.1f} "
          f"{s_dj['avg_travel_time']:>10.1f} {float(nz)*100:>13.1f}%")

print("\n=== means over held-out seeds ===")
for k, v in arms.items():
    print(f"  {k:16s}: {mean(v):7.1f}s")
print(f"\nReferences (measured): T_static {T_STATIC:.0f} | T_DUE {T_DUE:.0f} (selfish eq) "
      f"| T_SO {T_SO:.0f} (optimum)")
mg_mean = mean(arms["MAPPO-greedy"])
frac = (T_DUE - mg_mean) / (T_DUE - T_SO) if (T_DUE - T_SO) else float("nan")
print(f"\nMAPPO-greedy sits at {mg_mean:.0f}s. Fraction of the coordination gap "
      f"(T_DUE->T_SO) captured: {frac*100:.0f}%")
