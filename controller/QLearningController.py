        ...
        for vehicle in vehicles:
            ...
            if vid in self._pending_decisions:
                ...
                if phase == "observe_lane_change":
                    ...
                    fallback_actions = self.decision_engine.lane_feasible_fallback_actions(obs_context, blocked_action=pending.intended_action)
                    if not fallback_actions:
                        fallback_actions = self.decision_engine.safe_connected_fallback_actions(obs_context, blocked_action=pending.intended_action)
                    if not fallback_actions:
                        continue
            ...
            if not safe_ok:
                ...
                if not fallback_actions:
                    fallback_actions = self.decision_engine.safe_connected_fallback_actions(context, blocked_action=action_idx)
                if not fallback_actions:
                    continue
